...
class MainCanvas(ttk.Frame):
    def __init__(self, parent, c_dimension, p_width, p_height, p_layers, *args, **kwargs):
       ...
       N    V-----V-----V-----V-----V
        ...
        4 - pixel mesh is a array containing the pixel vertices of a level
        5 - pixel lattice is an array containing each hexcolor pixel grid for each level
        6 - pixel stack is an array of the individual pixel sheets
        ...
        self.canvas.create_polygon(self.pixel_sheet,
                                    fill = '',
                                    outline='blue',
                                    tags=("shapes")
                                    )
    ...
    def scale_sprite_layer(self, event):
        ...
        if prev_scale != self.pixel_scale:
            ...
            #if draw_area:
            for i in range(0, len(self.pixel_sheet), 2):
                x, y = translate_2D(-cx, -cy, self.pixel_sheet[i], self.pixel_sheet[i + 1])
                x, y = scale_2D(dir, dir ,x, y)
                self.pixel_sheet[i], self.pixel_sheet[i + 1] = translate_2D(cx, cy, x, y)
    ...
    def rotate_sprite_layer(self, *args):
        ...
        #cx = self.pixel_sheet[4] - self.pixel_sheet[0]
        #cy = self.pixel_sheet[5] - self.pixel_sheet[1]
    ...
    def erase(self, event):
        ...
        if polygon_point(self.pixel_sheet, x, y):
            ...
            key_coord = self.coord_str(grid_x, grid_y)
    ...
    def draw_drag(self, event): 
        ...
        if polygon_point(self.pixel_sheet, x, y):
            ...
            if grid_x != self.last_pixel[0] and grid_y != self.last_pixel[1]:
                ...
                #coord_keys = [self.coord_str(coord[0], coord[1]) for coord in between]
                #print(between)
                for i in range(len(between)):
                    
                    x_pixel, y_pixel = between[i]
                    key_coord = self.coord_str(between[i][0], between[i][1])
                    vertices = self.pixel_coords[key_coord]
                    top_left  = vertices[0]
                    top_right  = vertices[1]
                    bottom_right = vertices[2]
...
class App(tk.Tk): 
    def __init__(self):
        ...
        self.mn_canvas = MainCanvas(self, 512, 8,8, 3)
        self.mn_canvas.grid()
