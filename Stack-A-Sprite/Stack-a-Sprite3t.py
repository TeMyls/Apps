...
class MainCanvas(ttk.Frame):
    ...
    def get_pixel_grid_coords(self, mouse_x, mouse_y, canvas_width, canvas_height):
        ...
        self.canvas.create_line(
            self.pixel_sheet[0], self.pixel_sheet[1],
            self.pixel_sheet[4], self.pixel_sheet[5], 
            fill = "pink",
            tags="lines"
        )
        '''
        ...
        col_X2, col_Y2 = self.DDA_raycast(self.pixel_sheet[0], self.pixel_sheet[1], self.degrees_to_radians(180 - self.angle), (grid_x + 1) * self.pixel_scale)
        col_X1, col_Y1 = self.DDA_raycast(self.pixel_sheet[0], self.pixel_sheet[1], self.degrees_to_radians(180 - self.angle), grid_x * self.pixel_scale)
        row_X2, row_Y2 = self.DDA_raycast(self.pixel_sheet[0], self.pixel_sheet[1], self.degrees_to_radians(270 -  self.angle), (grid_y + 1) * self.pixel_scale)
        row_X1, row_Y1 = self.DDA_raycast(self.pixel_sheet[0], self.pixel_sheet[1], self.degrees_to_radians(270 -  self.angle), grid_y * self.pixel_scale)

        top_left_x, top_left_y = self.DDA_raycast(col_X1, col_Y1, self.degrees_to_radians(270 -  self.angle), (grid_y) * self.pixel_scale)
        bottom_right_x, bottom_right_y = self.DDA_raycast(col_X2, col_Y2, self.degrees_to_radians(270 -  self.angle), (grid_y + 1) * self.pixel_scale)
        top_right_x, top_right_y  = self.DDA_raycast(row_X1, row_Y1, self.degrees_to_radians(180 -  self.angle), (grid_x + 1) * self.pixel_scale)
        bottom_left_x, bottom_left_y = self.DDA_raycast(row_X2, row_Y2, self.degrees_to_radians(180 -  self.angle), (grid_x) * self.pixel_scale)
        if True:
        ...
        '''
        #gx, gy = self.DDA_raycast(self.pixel_sheet[0], self.pixel_sheet[1], self.degrees_to_radians(270 - self.angle), (grid_y + 1) * self.pixel_scale)
        '''
        self.canvas.create_line(
                self.pixel_sheet[0], self.pixel_sheet[1],
                gx, gy, 
        ...
        '''
        self.canvas.create_line(
            grid_x * self.pixel_scale * math.cos(self.degrees_to_radians(self.angle)), grid_x * self.pixel_scale * math.sin(self.degrees_to_radians(self.angle)),
            mouse_x + math.cos(-self.degrees_to_radians(90 - angle)) * line_len_x, mouse_y + math.sin(self.degrees_to_radians(90 - angle)) * line_len_y, 
            fill = "pink",
            tags="lines"
        )
        ...
        #left line
        self.canvas.create_line(
            grid_y * self.pixel_scale * math.cos(self.degrees_to_radians(self.angle)), grid_y * self.pixel_scale * math.sin(self.degrees_to_radians(self.angle)),
            mouse_x + math.cos(self.degrees_to_radians(-angle)) * line_len_x, mouse_y + math.sin(self.degrees_to_radians(-angle)) * line_len_y, 
            fill = "cyan",
            tags="lines"
        )
        '''
...
class App(tk.Tk): 
    ...
    def __init__(self):
        ...
        self.mn_canvas = MainCanvas(self, 512, 6, 6, 1)
        self.mn_canvas.grid()
