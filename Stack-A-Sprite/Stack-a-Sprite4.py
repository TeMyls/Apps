...
class MainCanvas(ttk.Frame):
    ...
    def get_existing_vertices(self, grid_x, grid_y, grid_w, grid_h):
    ...
    def apply_stack_transformations(self,
        ...
        dim_max = max(len(self.pixel_tops[0]), len(self.pixel_left[0]))
        dim_min = min(len(self.pixel_tops[0]), len(self.pixel_left[0]))
        sheet = []
        prev_sheet = []
...
class App(tk.Tk): 
    def __init__(self):
        ...
        self.mn_canvas = MainCanvas(self, 512, 8, 8, 8)
        self.mn_canvas.grid()
