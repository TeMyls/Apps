...
class MainCanvas(ttk.Frame):
    def __init__(self, parent, c_dimension, p_width, p_height, p_layers, *args, **kwargs):
       ...
       N    V-------V-------V-------V-------V  
        ...
        5 - pixel top is an array of  the top vertex XY coordinates, althought they are not Vertex objects
        6 - pixel left is an array of  the left vertex XYcoordinates, althought they are not Vertex objects

        
        5 - pixel area is a array containing the pixel coords of a level
        ...
        8 - pixel tops is an array of pixel tops for each layer
        9 - pixel lefts is an array of pixel lefts for each layer
    ...
    def create_sprite_stack(self,c_dimension):
        ...
        for z in range(self.pixel_canvas_layers):
            ...
            if z < 1:
                ...
                self.translate_scl.config(
                                        to = round(self.max_pixel_scale * self.pixel_canvas_layers * self.pixel_canvas_height), 
                                         from_=-1 * round(self.max_pixel_scale * self.pixel_canvas_layers * self.pixel_canvas_height)
                                        )
                self.translate_scl.set(0)
                print(self.translate_scl.cget('to'))
            ...
            for y in range(self.pixel_canvas_height + 1):
                ...
                for x in range(self.pixel_canvas_width + 1):
                    ...
                    if y == 0 or x == 0:
                        if x == 0:
                            pixel_left.append([vx, vy])
                        if y == 0:
                            pixel_top.append([vx, vy])
                            #print(len(vertices))
                        

                                #vertices.clear()
            #self.pixel_vertices = vertices
            self.pixel_tops.append(pixel_top)
            self.pixel_lefts.append(pixel_left)
            #self.pixel_mesh.append(vertices)
            self.pixel_mesh.append([])
        ...
        #self.focus_sprite_stack()
        #self.space_sprite_stack()
        self.pixel_grid  = self.pixel_lattice[self.pixel_level]
        self.pixel_coords = self.pixel_area[self.pixel_level]
        self.pixel_left = self.pixel_lefts[self.pixel_level]
        self.pixel_top = self.pixel_tops[self.pixel_level]
        #self.pixel_vertices = self.pixel_mesh[self.pixel_level]
        self.pixel_sheet = self.pixel_stack[self.pixel_level]
    ...
    def get_vertices(self, grid_x, grid_y, itr, ibr, ibl, itl):
    ...
    def render_stack_levels(self):
        ...
        for i in range(len(self.pixel_area) -1, -1, -1):
            
            if self.pixel_area[i]:
                for key in self.pixel_area[i]:
                    ...
                    '''  
                    if i == cur_level:
                        tops = self.pixel_tops[i]
                        lefts = self.pixel_lefts[i]
                        dim_max = max(len(self.pixel_tops[0]), len(self.pixel_lefts[0]))
                        dim_min = min(len(self.pixel_tops[0]), len(self.pixel_lefts[0]))
      

                        self.canvas.delete("layerlines")
                        for k in range(dim_max):
                            ...
                            if k < len(tops):
                                self.canvas.create_line(
                                    tops[k][0], 
                                    tops[k][1],
                                    tops[k][0] - math.cos(self.degrees_to_radians(90 - self.angle)) * self.pixel_scale * self.pixel_canvas_width, 
                                    tops[k][1] - math.sin(self.degrees_to_radians(90 - self.angle)) * self.pixel_scale * self.pixel_canvas_width, 
                                    fill = "black",
                                    tags="layerlines"
                                )
                            ...
                            if k < len(lefts):
                                self.canvas.create_line(
                                    lefts[k][0], 
                                    lefts[k][1],
                                    lefts[k][0] - math.cos(self.degrees_to_radians(-self.angle)) * self.pixel_scale * self.pixel_canvas_height, 
                                    lefts[k][1] - math.sin(self.degrees_to_radians(-self.angle)) * self.pixel_scale * self.pixel_canvas_height, 
                                    fill = "black",
                                    tags="layerlines"
                                )
    ...
    def apply_stack_transformations(self,
                                    is_rotating, 
                                    is_scaling, 
                                    is_spacing, 
                                    is_focusing, 
        ...
        for i in range(len(self.pixel_stack)):
            ...
            if is_rotating:
                ...
                #print("spaced")
                cx, cy = line_line_intersection(
                    self.pixel_stack[i - 1][0], self.pixel_stack[i - 1][1],
                    self.pixel_stack[i - 1][4], self.pixel_stack[i - 1][5],
                    self.pixel_stack[i - 1][2], self.pixel_stack[i - 1][3],
                    self.pixel_stack[i - 1][6], self.pixel_stack[i - 1][7]
                ) 
            ...
            for j in range(0, len(sheet), 2):
                ...
                if is_scaling:
                    ...
                    if i != self.pixel_level:
                        x, y = translate_2D(0, (self.pixel_level - i)  * spacing_change, x, y)
                elif is_focusing:
                    x, y = translate_2D(0, focus_change, x, y)
            ...
            for k in range(dim_max):
                ...
                if k < len(tops):
                    x, y = translate_2D(-cx, -cy, tops[k][0], tops[k][1])
                    ...
                    tops[k][0], tops[k][1] = translate_2D(cx, cy, x, y)
                    

                
                if k < len(lefts):
                    x, y = translate_2D(-cx, -cy, lefts[k][0], lefts[k][1])
                    ...
                    lefts[k][0], lefts[k][1] = translate_2D(cx, cy, x, y)
    ...
    def scale_sprite_stack(self, event):
        ...
        if prev_scale != self.pixel_scale:
            self.apply_stack_transformations(False, True, False, False, 0, scaling_change, 0, 0)
    ...
    def rotate_sprite_stack(self, *args):
        ...
        self.apply_stack_transformations(True, False, False, False, angle_change, 0, 0, 0)
    ...
    def space_sprite_stack(self, *args):
        ...
        self.apply_stack_transformations(False, False, True, False, 0, 0, spacing_change, 0)
    ...
    def focus_sprite_stack(self, *args):
        ...
        tops = self.pixel_tops[self.pixel_level]
        lefts = self.pixel_lefts[self.pixel_level]
        dim_max = max(len(self.pixel_tops[0]), len(self.pixel_lefts[0]))
        #dim_min = min(len(self.pixel_tops[0]), len(self.pixel_lefts[0]))
        ...
        #self.apply_stack_transformations(False, False, False, True, 0, 0, 0,  focus_change)
    ...
    def translate_y_stack(self, *args):
        ...
        focus_change = (prev_translation - self.translation) #* (self.pixel_scale * self.pixel_canvas_height)
        self.apply_stack_transformations(False, False, False, True, 0, 0, 0,  focus_change)
    ...
    def save(self):
        ...
        grid_pixel = []
        row = 0
        for z in range(len(self.pixel_lattice)):
            coords = self.pixel_lattice[z]
            for y in range(len(self.pixel_lattice[z])):
                grid_pixel.append([])
                for x in range(len(self.pixel_lattice[y])):
                    #each in rgb alpha
                    if coords[y][x] != None:
    ...
    def display_data(self):
        print("Data for current layer")

        print("Current Pixel Grid")
        self.display_2d(self.pixel_grid)
        print("Current Pixel Coords")
        for key in self.pixel_coords:
            print(key, list(map(str, self.pixel_coords[key])))
            #break
        
        print("Current Pixel top")
        self.display_2d(self.pixel_top)
        print("Current Pixel left")
        self.display_2d(self.pixel_left)
        print("Current Color")
        print(self.selected_color)
        print("Current Pixel Vertices")
        for vert in self.pixel_vertices:
            print(vert)
        print("Current Vertex Layer Amount: ",len(self.pixel_vertices))
    ...
    def get_pixel_grid_coords(self, mouse_x, mouse_y, canvas_width, canvas_height):
        ...
        top_left = self.in_bounds(grid_x, grid_y, self.pixel_canvas_width, self.pixel_canvas_height)
        top_right = self.in_bounds(grid_x + 1, grid_y, self.pixel_canvas_width, self.pixel_canvas_height)
        bottom_right = self.in_bounds(grid_x + 1, grid_y + 1, self.pixel_canvas_width, self.pixel_canvas_height)
        bottom_left = self.in_bounds(grid_x, grid_y + 1, self.pixel_canvas_width, self.pixel_canvas_height)
        ...
        if False:
            #box top
            self.canvas.create_line(
                self.pixel_left[grid_y][0], self.pixel_left[grid_y][1],
                self.pixel_left[grid_y][0] - math.cos(-self.degrees_to_radians(-self.angle)) * line_len_y, self.pixel_left[grid_y][1] - math.sin(self.degrees_to_radians(-self.angle)) * line_len_y, 
                fill = "yellow",
                tags="lines"
            )
            ...
            self.canvas.create_line(
                self.pixel_top[grid_x][0], self.pixel_top[grid_x][1],
                self.pixel_top[grid_x][0] - math.cos(-self.degrees_to_radians(90 - self.angle)) * line_len_x, self.pixel_top[grid_x][1] - math.sin(self.degrees_to_radians(90 - self.angle)) * line_len_x, 
                fill = "cyan",
                tags="lines"
            )
            ...
            #box right
            self.canvas.create_line(
                self.pixel_top[grid_x + 1][0], self.pixel_top[grid_x + 1][1],
                self.pixel_top[grid_x + 1][0] - math.cos(-self.degrees_to_radians(90 - self.angle)) * line_len_x, self.pixel_top[grid_x + 1][1] - math.sin(self.degrees_to_radians(90 - self.angle)) * line_len_x, 
                fill = "magenta",
                tags="lines"
            )
            ...
            #box bottom
            self.canvas.create_line(
                self.pixel_left[grid_y + 1][0], self.pixel_left[grid_y + 1][1],
                self.pixel_left[grid_y + 1][0] - math.cos(-self.degrees_to_radians(-self.angle)) * line_len_y, self.pixel_left[grid_y + 1][1] - math.sin(self.degrees_to_radians(-self.angle)) * line_len_y, 
                fill = "black",
                tags="lines"
            )
        ...
        intersect_top_left = line_line_intersection(
            #Top
            self.pixel_left[grid_y][0], self.pixel_left[grid_y][1],
            self.pixel_left[grid_y][0] - math.cos(-self.degrees_to_radians(-self.angle)) * line_len_y, self.pixel_left[grid_y][1] - math.sin(self.degrees_to_radians(-self.angle)) * line_len_y, 
            #Left
        ...
        intersect_top_right = line_line_intersection(
            #Top
            self.pixel_left[grid_y][0], self.pixel_left[grid_y][1],
            self.pixel_left[grid_y][0] - math.cos(-self.degrees_to_radians(-self.angle)) * line_len_y, self.pixel_left[grid_y][1] - math.sin(self.degrees_to_radians(-self.angle)) * line_len_y, 
            #Right
        ...
        intersect_bottom_left = line_line_intersection(
            #Top
            self.pixel_left[grid_y + 1][0], self.pixel_left[grid_y + 1][1],
            self.pixel_left[grid_y + 1][0] - math.cos(-self.degrees_to_radians(-self.angle)) * line_len_y, self.pixel_left[grid_y + 1][1] - math.sin(self.degrees_to_radians(-self.angle)) * line_len_y,
            #Left
        ...
        intersect_bottom_right = line_line_intersection(
            #Top
            self.pixel_left[grid_y + 1][0], self.pixel_left[grid_y + 1][1],
            self.pixel_left[grid_y + 1][0] - math.cos(-self.degrees_to_radians(-self.angle)) * line_len_y, self.pixel_left[grid_y + 1][1] - math.sin(self.degrees_to_radians(-self.angle)) * line_len_y,
            #Right
        ...
        return grid_x, grid_y, intersect_top_left, intersect_top_right, intersect_bottom_right, intersect_bottom_left

    def clear(self):
        for y in range(self.pixel_canvas_height):
            for x in range(self.pixel_canvas_width):
                key_coord = self.coord_str(x, y, self.level.get())
                if self.pixel_coords.get(key_coord):
                    self.pixel_coords[key_coord].clear()
    ...
    def draw(self, event): 
        ...
        if polygon_point(self.pixel_sheet, x, y):
            #print('yep')
            grid_x, grid_y, itl, itr, ibr, ibl = self.get_pixel_grid_coords(x, y, canvas_width, canvas_height)
            key_coord = self.coord_str(grid_x, grid_y, self.level.get())
...
class App(tk.Tk): 
    def __init__(self):
        ...
        self.mn_canvas = MainCanvas(self, 512, 8, 8, 4)
        self.mn_canvas.grid()
