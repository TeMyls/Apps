       ...
       """
        ...
        for i in range(len(bottom_right_vectors)):
            if self.in_bounds(grid_x + bottom_right_vectors[i][0], grid_y + bottom_right_vectors[i][1], self.pixel_canvas_width, self.pixel_canvas_height):
                coord_key = self.coord_str(grid_x + bottom_right_vectors[i][0], grid_y + bottom_right_vectors[i][1], self.z_layer)
        ...
        #for i in range(len(bottom_left_vectors)):
            if self.in_bounds(grid_x + bottom_left_vectors[i][0], grid_y + bottom_left_vectors[i][1], self.pixel_canvas_width, self.pixel_canvas_height):
                coord_key = self.coord_str(grid_x + bottom_left_vectors[i][0], grid_y + bottom_left_vectors[i][1], self.z_layer)
                if self.pixel_coords.get(coord_key):
                    vertices = self.pixel_coords[coord_key]
                    if i == 0: #West
        ...
        #for i in range(len(top_left_vectors)):
            if self.in_bounds(grid_x + top_left_vectors[i][0], grid_y + top_left_vectors[i][1], self.pixel_canvas_width, self.pixel_canvas_height):
                coord_key = self.coord_str(grid_x + top_left_vectors[i][0], grid_y + top_left_vectors[i][1], self.z_layer)
                if self.pixel_coords.get(coord_key):
                    vertices = self.pixel_coords[coord_key]
                    if i == 0: #North
                        n_bottom_left  = vertices[3]
        ...
        #for i in range(len(top_right_vectors)):
            if self.in_bounds(grid_x + top_right_vectors[i][0], grid_y + top_right_vectors[i][1], self.pixel_canvas_width, self.pixel_canvas_height):
                coord_key = self.coord_str(grid_x + top_right_vectors[i][0], grid_y + top_right_vectors[i][1], self.z_layer)
                if self.pixel_coords.get(coord_key):
                    vertices = self.pixel_coords[coord_key]
...
class MainCanvas(ttk.Frame):
    def __init__(self, parent, c_dimension, p_width, p_height, p_layers, *args, **kwargs):
        ...
        self.add_button = ttk.Button(self, text="+", command=None)
        self.sub_button = ttk.Button(self, text="-", command=None)
        ...
        arrangement = [
                ...
                [None                , self.import_button  , None                , self.visible_layer_cb   ],
                [None                , self.add_button     , self.sub_button     , self.visible_grid_cb    ]
        ]
    ...
    def create_sprite_stack(self):
        ...
        ratio_y = 0
        for z in range(self.pixel_canvas_layers):
            last_stack = self.sprite_stack[-1]
            '''
            self.top_left.get_X(), self.top_left.get_Y(),
            self.bottom_right.get_X(), self.bottom_right.get_Y(),
            self.bottom_left.get_X(), self.bottom_left.get_Y(),
            ...
            #top left
            x0 = last_stack.top_right.get_X()
            y0 = last_stack.top_right.get_Y()
            #bottom left
            x1 = last_stack.bottom_right.get_X()
            y1 = last_stack.bottom_right.get_Y()
            #bottom right
            x2 = x0 + (last_stack.top_right.get_X() - last_stack.top_left.get_X())
            y2 = y0 + (last_stack.bottom_right.get_Y() - last_stack.top_right.get_Y())
            #top right
            x3 = x0 + (last_stack.top_right.get_X() - last_stack.top_left.get_X())
            y3 = y0 
    ...
    def clear(self):
        ...
        self.sprite_sheet.pixel_vertices = []
        for y in range(self.pixel_canvas_height):
            for x in range(self.pixel_canvas_width):
                key_coord = self.sprite_sheet.coord_str(x, y, self.level.get())
                self.canvas.delete(key_coord)
                
                self.sprite_sheet.pixel_coords[key_coord] = []
    ...
    def erase(self, event):
        ...
        if polygon_point(self.sprite_sheet.get_borders(), x, y):
            #print('yep')
            self.sprite_sheet.paint_pixel(x, y, canvas_width, canvas_height, -self.angle, self.pixel_scale, self.canvas, self.canvas_bg_color, True)
            self.render_stack_levels()
    ...
    def draw_drag(self, event): 
        ...
        if polygon_point(self.sprite_sheet, x, y):
            #print('yep')
            cur_level = self.level.get()
            self.sprite_sheet.paint_pixel(x, y, canvas_width, canvas_height, -self.angle, self.pixel_scale, self.canvas, self.selected_color, False)
            key_coord = self.coord_str(self.sprite_sheet.grid_x, self.sprite_sheet.grid_y, cur_level)
                    
                
            #print("drag", self.last_pixel[0], self.last_pixel[1], grid_x, grid_y)
            
            if self.sprite_sheet.grid_x != self.last_pixel[0] and self.sprite_sheet.grid_y != self.last_pixel[1]:
                     
                between = self.sprite_sheet.DDA(self.last_pixel[0], self.last_pixel[1], self.sprite_sheet.grid_x, self.sprite_sheet.grid_y)
                ...
                for i in range(len(between)):
                    ...
                    key_coord = self.sprite_sheet.coord_str(between[i][0], between[i][1], cur_level)
                    ...
                    self.sprite_sheet.pixel_grid[y_pixel][x_pixel] = self.selected_color
                    #self.pixel_coords[current_coord] = True
                    #self.display_2d(self.pixel_grid)
                ...
                #print(between)
                self.render_stack_levels()
                self.last_pixel = [self.sprite_sheet.grid_x, self.sprite_sheet.grid_y]
...
class App(tk.Tk): 
    def __init__(self):
        ...
        self.mn_canvas = MainCanvas(self, 512, 64, 64, 2)
        self.mn_canvas.grid()
