...
class ImageFrame():
    ...
    # render_image
        # actually draws the canvas_image to the canvsa
            ...
            if in_bounds(x_pixel, y_pixel, self.pixel_canvas_width, self.pixel_canvas_height):
                if not self.is_drawing_line:
                    ...
                    for pixel in self.stroke_pixels:
                        #current_color = self.hex_to_rgb(color, True)
                        #a very light alpha value signaling the pixel is a part of stroke pixels
                        #and should be deleted until the mouse is released
                        #current_color[-1] = 1
                        
                        #self.pixel_image.putpixel((pixel[0], pixel[1]), tuple(current_color))
                    ...
                    for i in range(len(self.stroke_pixels)):
                        pixel = self.stroke_pixels[i]
        ...
        if polygon_point(borders, x, y):
            ...
            try:
                ...
                if rect_center:
                    ...
                    ellipse_points = mid_point_ellipse(rect_center[0], rect_center[1], crw, crh)
        ...
        if polygon_point(borders, x, y):
            ...
            for y in range(self.pixel_canvas_height):
                for x in range(self.pixel_canvas_width):
                    ...
                    rgba = self.pixel_image.getpixel((x, y))
                    if rgba[3] != 0 and (x, y) not in self.temp_pixels and [x, y] not in self.temp_pixels:
                        #print("ye")
                        current_color = self.rgb_to_hex(rgba[0], rgba[1], rgba[2])
                        current_coord = self.coord_to_str(x, y)
                        self.draw_pixel(canvas, current_coord, current_color, current_color, current_coord, borders)
