...
class ImageFrame():
    ...
    def pixel_to_pil_image(self, default_color = [255, 255, 255, 0]):
        #print("pixel image")
        ...
        grid_pixel = [[  default_color  ] * len(self.pixel_grid[0]) for _ in range(len(self.pixel_grid))]
    ...
    def alter_image_dimensions(self, anchor, new_width, new_height, canvas, borders):
        ...
        cur_width, cur_height = self.pixel_canvas_width, self.pixel_canvas_height

        width_ratio = {

            "NW": 0.00, "N": 0.33, "NE": 0.66,
            "W":  0.00, "C": 0.33,  "E": 0.66,
            "SW": 0.00, "S": 0.33, "SE": 0.66

        }

        height_ratio = {

            "NW": 0.00, "N": 0.00, "NE": 0.00,
            "W":  0.33, "C": 0.33,  "E": 0.33,
            "SW": 0.66, "S": 0.66, "SE": 0.66
        ...
        blank_image.paste(self.pixel_image, (round(new_width * width_ratio[anchor]), round(new_height * height_ratio[anchor])))
    ...
    def brush_click(self, x, y, canvas, color, borders, is_drawing):
        #print("draw erase 1")
    ...
    def brush_press(self, x, y, canvas, color, borders, is_drawing):
        #print(x, y)
        #print("draw erase")
    ...
    def end_stroke(self, x, y, canvas, color, borders):
        ...
        if self.is_drawing_line:
            ...
            if self.stroke_pixels:
                ...
                current_coord = self.coord_to_str(self.first_pixel[0], self.first_pixel[1])
                self.draw_pixel(canvas, current_coord, color, color, current_coord, borders) 
                self.pixel_image.putpixel((self.first_pixel[0], self.first_pixel[1]), tuple(self.hex_to_rgb(color)))
                    ...
                    if x_pixel >= 0 and y_pixel >= 0 and x_pixel < self.pixel_canvas_width and y_pixel < self.pixel_canvas_height:
                        ...
                        if self.last_pixel:
                            ...
                            if color:
                                ...
                                self.pixel_image.putpixel((old_px, old_py), self.blank)
                                self.pixel_image.putpixel((new_px, new_py), color)
    ...
    def select_release(self, x, y, canvas, color, borders):
        ...
        if polygon_point(borders, x, y):
            #self.temp_colors = [self.pixel_grid[pxl[1]][pxl[0]] for pxl in self.temp_pixels]
            self.temp_colors = [self.pixel_image.getpixel((pxl[0], pxl[1])) for pxl in self.temp_pixels]
    ...
    def bucket_fill(self, x, y, canvas, color, borders):
        ...
        for row in range(im_height):
            for col in range(im_width):
                if self.pixel_image.getpixel((col, row)):
                    
                    current_coord = self.coord_to_str(col, row)
                    self.draw_pixel(canvas, current_coord, color, color, current_coord, borders) 
                    canvas.update_idletasks()
...
# -------------------------------------------------------------------------------
    ...
    def translate(self, offset_x, offset_y,zoom = False):
        ...
        im_bounds_w = self.pixel_image.width
        im_bounds_h = self.pixel_image.height
    ...
    def to_image_point(self, x, y):
        ...
        if  image_point[0] < 0 or image_point[1] < 0 or image_point[0] > self.pixel_image.width or image_point[1] > self.pixel_image.height:
            return []
    ...
    def draw_image(self, pil_image):
        ...
        dst = self.pixel_image.transform(
    ...
    def redraw_image(self):
        ...
        if self.pixel_image == None:
            return
        self.draw_image(self.pixel_image)

        translation_matrix = np.eye(3) #np.array(translation_matrix2D(border_width/2, border_height/2))
        transform_matrix = np.array(scale_matrix2D(scale_dim * im_width, scale_dim * im_height))
        matrix_translation = np.eye(3) #np.array(translation_matrix2D(-border_width/2, -border_height/2))
        
        full_transform = matrix_translation @ transform_matrix @ translation_matrix

        full_transform = np.linalg.inv(full_transform)
        #full_transform = (col for row in full_transform for col in row)
        
        # Convert the numpy array to a tuple for affine transformation
        
        full_transform = (
            full_transform[0, 0], full_transform[0, 1], full_transform[0, 2],
            full_transform[1, 0], full_transform[1, 1], full_transform[1, 2],
            full_transform[2, 0], full_transform[2, 1], full_transform[2, 2]
        )
        # Apply affine transformation to the PIL image data
        self.canvas_image = self.pixel_image.transform(
            (round(border_width), round(border_height)),  # Output size
            Image.AFFINE,   # Affine transformation
            full_transform,     # Affine transformation matrix (conversion matrix from output to input)
            Image.Resampling.NEAREST   # Interpolation method, nearest neighbor
        )

        print(self.canvas_image.size)
        print(scale_dim * im_width, scale_dim * im_height)
