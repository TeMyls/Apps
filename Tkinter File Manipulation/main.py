...
class PathForm(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.convert_to_combobox["values"] = (  ""
                                                "mp4",
                                                "ogv",
                                                "webm",
                                                "gif",
        ...
        self.save_path_button = tk.Button(self, text="Save File(s)", command=self.save_files)
        ...
        arrangement = [
            ...
            [self.selected_file_open_button, None                          , self.cut_label          , None                   , self.audio_label         , None                 , self.save_path_button       ],
            [None                          , self.start_cut_label          , self.cut_start_spinbox  , None                   , self.mute_checkbutton    , self.muted_label     , self.save_name_label        ],                     
            [None                          , self.end_cut_label            , self.cut_end_spinbox    , None                   , None                     , None                 , self.save_name_entry        ]                           
            ...
            for ind_x in range(len(arrangement[ind_y])):
                ...
                if arrangement[ind_y][ind_x] == None:
                    ...
                    if isinstance(arrangement[ind_y][ind_x], tk.Label):
                        ...
                        arrangement[ind_y][ind_x].grid(row = ind_y, column = ind_x, sticky = "NWS")
    ...
    def open_files(self):
        ...
        if file_path_list:
            #text = "\n".join(file_path_list)
            for ind in range(self.selected_file_listbox.size()):
                self.selected_file_listbox.delete(ind)
            
            for ind in range(len(file_path_list)):
                edited_file = file_path_list[ind].split('/')[-1]
                self.selected_file_listbox.insert(ind, edited_file)
