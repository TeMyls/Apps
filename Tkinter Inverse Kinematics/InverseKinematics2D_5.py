...
class Kinematics(ttk.Frame):
    def __init__(self, parent, c_width, c_height):
        ...
        self.rotation_lbl =  ttk.Label(self.bottom_frame, text = "Rotation", anchor="center")
        ...
        self.rotation_lbl.pack(side="bottom", anchor="center", expand=True)
        self.rotation_scl.pack(side="bottom", anchor="center", expand=True)
        ...
        self.exp_btn.pack(side="top", fill="x", expand=True, anchor="center")


        self.vertex_tv = ttk.Treeview(self.data_frame, height=15)
        self.vertex_tv.pack(side="top", fill="y", expand=True, anchor="center")
        #------------------------------------------------------------------------------------------------
        ...
        btn_arrangement = [
            ...
            [self.shift_btn, self.select_btn],
            [self.connect_btn, self.break_btn],
            [self.target_btn, self.erase_btn]
    ...
    def on_canvas_resize(self, event):
        ...
        self.rotation_scl.config(length=self.canvas_width + 128)
    ...
    def display_data(self):
        ...
        print(f"Target: {self.target_idx}")
            ...
            if idx > -1 and self.selected_idx > -1:
                ...
                if idx != self.selected_idx:
                    ...
                    if idx not in self.edges[self.selected_idx]:
                        ...
                        cycle_count = len(get_cycles(self.edges))
                        #print(cycle_count)
                        if cycle_count > 0:
    ...
    def rotate_relative(self, *args):
        if self.vertices:
            ...
            childern = dfs(self.edges, self.selected_idx)[1:]
            for vtx in childern:
        ...
        for key1 in self.edges:
            ...
            for key2 in self.edges:
                ...
                if key1 != key2:
                    ...
                    distances[key1][key2] = distance_to(vx1, vy1, vx2, vy2)
    ...
    def reaching(self, v_chain, target_x, target_y, og_positions):
        ...
        for i in range(0, len(v_chain) - 1):
            ...
            angle = angle_to(vx1, vy1, vx2, vy2)
            ...
            if len(self.edges[v_chain[i]]) > 1:
                if v_chain[i] != v_chain[0]:
                    chain = [v_chain[i]] + list(set(dfs(self.edges, v_chain[i])) - set(v_chain))
    ...
    def FABRIK(self):
        if len(self.vertices) > 1 and self.target_idx != -1 and self.target_idx != self.selected_idx:
            base = self.selected_idx
            end = self.target_idx #random.choice([key for key in self.edges if not self.edges[key]])
            ...
            v_chain = dfs_path_between(self.edges, base, end)[::-1]
            #print(v_chain)
            if v_chain:
            ...
            #childern = dfs(self.edges, self.selected_idx)
        
    
    #Vertex Frame ---------------------------------------------------------------------------------------------------

    def treeview_click_bind(self, event):
        print(event, self.vertex_tv.selection(), self.vertex_tv.focus())


    def update_treeview(self, graph, node):
        if self.selected_idx != -1:
            
            for item in graph:
                str_item = str(item)
                if self.vertex_tv.exists(str_item):
                    self.vertex_tv.delete(str(item))
            
            # tree root
            self.vertex_tv.insert("", 
                                  "end", 
                                  str(node), 
                                  text=str(node), 
                                  #values=(f"{str(self.vertices[node])}")
                                  )
