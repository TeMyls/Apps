...
class MainCanvas(ttk.Frame):
    ...
    def greedy_mesh_flood_fill(self, row, col, color):
        pass
...
class ManipulationOptions(ttk.Frame):
    def __init__(self, parent):
        ...
        self.pref_cb = tk.Checkbutton(self, text="Preferences", variable=self.pref_val) 

        self.resize_val = tk.BooleanVar(value=False)
        self.resize_cb = tk.Checkbutton(self, text="Resize")  

        self.save_val = tk.BooleanVar(value=False)
        self.save_cb = tk.Checkbutton(self, text="Resize")  

        self.imp_val = tk.BooleanVar(value=False)
        self.imp_cb = tk.Checkbutton(self, text="Import")  

        self.exp_val = tk.BooleanVar(value=False)
        self.exp_cb = tk.Checkbutton(self, text="Export")  
        ...
        self.pref_cb.config(command=lambda:self.manage_widget_group(self.manip_widgets, self.pref_cb))
        self.pref_cb.config(command=lambda:self.manage_widget_group(self.manip_widgets, self.pref_cb))
        self.pref_cb.config(command=lambda:self.manage_widget_group(self.manip_widgets, self.pref_cb))
        self.pref_cb.config(command=lambda:self.manage_widget_group(self.manip_widgets, self.pref_cb))
        self.pref_cb.config(command=lambda:self.manage_widget_group(self.manip_widgets, self.pref_cb))
        ...
        arrange_widgets(self.manip_widgets)



    
    
    def manage_widget_group(self, manip_widget_arr, clicked_widget):
        for widget_bv in manip_widget_arr:
            widget = widget_bv[0]
            bv = widget_bv[1]
...
class DrawingTools(ttk.Frame):
    def __init__(self, parent):
        ...
        self.fill_img = tk.PhotoImage(file=r"icons\\format_color_fill_32dp_000000_FILL0_wght400_GRAD0_opsz40.png").subsample(4,4)
...
class App(tk.Tk): 
    def __init__(self):
        ...
        #self.palette.grid()
        arrangement = [
            [self. drawing_tools, self.drawing_canvas, self.palette],
            [None, None]
        ]
